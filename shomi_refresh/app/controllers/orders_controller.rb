class OrdersController < ApplicationController
  ...
  def index
    redirect_to promo_contact_path unless @current_session.profile.purchased? || @current_session.profile.eligible? || !session[:promo_code]
    redirect_to new_order_path(order: { billing_option: 'cc'}) unless @current_session.profile.accounts.any?

    @order = Order.new(optin: @current_session.profile.try(:optin), optin_account_token: @current_session.profile.try(:optin_account_token))
  ...
  def promo_contact
    ...
    if session[:promo_code] && !@current_session.profile.eligible? && !@current_session.profile.purchased?
      @order = extract_object(:order) || Order.new(@current_session.profile.attributes.merge({billing_option: "free", offer_id: @offer.offer_id, promo_code: session[:promo_code].code, code_type: session[:promo_code].code_type}))
  ...
  def order_params
    params.require(:order).permit(:phone_number, :promo_code, :code_type, :full_name, :first_name, :last_name, :street, :city, :postal_code, :billing_option, :province, :cc_token, :auto_renew,
                                  :optin, :optin_account_token, :tos, :tos2, :nhl_opt_in, :rogers_account_token, :offer_id, :optin_email, :masked_cc, :cc_type, :validate, :birthdate, birthdate: []).symbolize_keys
  end
end
