class Order < Portal
  ...
  encrypted :billing_option ,:province ,:street ,:postal_code ,:city ,:first_name ,:last_name ,:full_name ,:masked_cc ,:cc_type ,:ctn ,:can ,:optin_email ,:promo_code ,:auto_renew ,:phone_number ,:price ,:taxes ,:frequency ,:desc_en ,:desc_fr, :optin, :optin_account_token
  ...
  validates_presence_of :phone_number,  :if => -> { billing_option == 'rogersbill' && !skip_phone_number_validation? }
  validates_format_of :phone_number, with: /\A\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})\z/, :if => -> { !skip_phone_number_validation? }
