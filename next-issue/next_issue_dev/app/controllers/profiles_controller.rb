class ProfilesController < ApplicationController
  before_action :set_profile, only: [:show, :edit, :update, :destroy, :edit_email_preferences]
  ...
  def index
    ...
    if !@current_session.profile.try(:eligible?) && @current_session.profile.try(:optin)
      redirect_to orders_path
    elsif @current_session.profile.try(:user_orders).blank? && ENV['ineligible_forward_to_purchase']
      #in next issue redirect users to order page if they are not eligible
      redirect_to new_order_path(order: { billing_option: @current_session.profile.accounts.any? ? 'rogersbill' : 'cc' })
    elsif session[:promo_code] && !@current_session.profile.try(:purchased?) && !@current_session.profile.try(:eligible?) # || @current_session.profile.cannot_buy?
    ...
    #NOTE: SOMEBODY PLEASE REFACTOR THIS MESS ONE DAY. TOO MANY IF ELSE STATEMENTS.
    if ENV['APP_NAME'] == 'gcl'
    ...
    if ENV['APP_NAME'] == 'nextissue'
      membership_type = "applyredemption"
  ...
  def get_current_orders
    # Setup API client to get the orders
  ...
  def new
    @offer = Offer.current_offer(offer_id: session[:offer_id]) if session[:offer_id]

    @profile = extract_object(:profile) || Profile.new
    @profile.email_address = params[:email_address]
    @tos = Tos.latest
  ...
  def update
    ...
    respond_to do |format|
      if @profile.valid? && !profile_params[:validate]
        background_notice!(t('sessions.retrieving_info'))
        HardWorker.perform_async(session.id, :edit_profile, profile_params.merge({ip_address: request.ip, user_region: session[:user_region] }))
  ...
  def promo_code
    ...
    dont_collect_promo_details = @current_session.logged? && (@current_session.profile.eligible? || @current_session.latest_order.status.downcase == 'cancelled') rescue false
    offer_type = params[:offer_type]
    offer_type = session[:offer_id] == ENV['premium_offer_id'] ? 'premium' : 'basic' if offer_type.blank?

    respond_to do |format|
      if params[:promo_code] && !params[:promo_code][:code].to_s.gsub(/\s/,'').blank?
        HardWorker.perform_async(session.id, :promo_code?, { code: params[:promo_code][:code], current_url: request.referer, billing_option: params[:billing_option], user_is_active: @current_session.try(:profile).try(:eligible?),
                                                             validate_redemption_only: params[:validate_redemption], dont_collect_promo_details: dont_collect_promo_details, latest_order_id: @current_session.try(:latest_order).try(:order_id),
                                                             offer_type: offer_type, operation: params[:operation], lang: I18n.locale })
        format.js { render 'pages/background_job' }
      else
        flash[:notice] = t('profiles.notice.not_valid_promo')
        flash[:notice_type] = :alert
        flash[:notice_is_promo_code] = 'invalid'
  ...
  def email_check
    if @current_session.try(:logged?)
      if @current_session.profile.eligible?
    ...
    if ENV['redemption_top_up'] && session[:promo_code] && !session[:promo_code].premium_redemption? && session[:promo_code].basic_redemption?
      # set offer id to basic if the redemption code is basic
      session[:offer_id] = ENV['basic_offer_id']
    end
    @offer = Offer.current_offer(offer_id: session[:offer_id])

    if params[:email_check]
      if params[:email_check][:email_address].blank? ||
          params[:email_check][:email_address].match(RubyRegex::Email).nil? ||
          !params[:validate]
        respond_to do |format|
          format.json { render json: { name: 'email_check',
                                       errors: {email_address: [t('errors.attributes.email_address.invalid')]} } }
        end
      else
        background_notice!(t('profiles.verifying_email_address'))
        HardWorker.perform_async(session.id, :email_check, {email_address: params[:email_check][:email_address], current_url: request.referer})
        respond_to do |format|
          format.js { render 'pages/background_job' }
        end
      end
