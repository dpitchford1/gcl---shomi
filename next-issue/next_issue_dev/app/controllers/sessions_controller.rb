class SessionsController < ApplicationController
  ...
  def new
    cookies[:flash] = 'null' if cookies.fetch(:flash, []).include? 'notice_is_promo_code'
  ...
  def cookie_sso    
    cookie_smsession = cookies[:SMSESSION].gsub(" ", "+")
    respond_to do |format|
      if cookies[:SMSESSION] && (cookies[:SMSESSION] != "LOGGEDOFF") && !current_session.logged?
        background_notice!(t('sessions.retrieving_info'))
        HardWorker.perform_async(session.id, :login, {username: "temp@temp.com", password: "password", lang: "en", smsession: cookie_smsession, ip_address: request.ip, user_region: session[:user_region]} )
        cookies.delete(:SMSESSION, domain: :all)

        format.js { render 'pages/background_job' }
      else
        format.js { render js: "" } #just return nothing
      end
    end
  end
