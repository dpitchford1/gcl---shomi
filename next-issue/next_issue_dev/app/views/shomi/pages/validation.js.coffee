addLoadEvent ->
  object_name = "<%= name %>"
  optin = "<%= @current_session.profile.try(:optin) rescue false %>"
    ...
    $.post "/<%= path %>", form.serialize() + '&<%= name %>[validate]=true&validate=true', (data) ->
      if validate_all
        ...
        for name of data.errors
          if optin == 'true' && (name == 'phone_number' || name == 'birthdate' || name == 'rogers_account_token')
            # shomi optin true, skip validating phone number & dob for orders form
          else
            errors_cnt += 1
