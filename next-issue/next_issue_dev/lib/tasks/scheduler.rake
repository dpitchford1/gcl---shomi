...
desc "Fetching survey questions"
task :fetch_survey_questions, [:survey_type] => [:environment] do |t, args|
  puts "Fetching survey questions...#{args.inspect}"
  SchedulerWorker.perform_async(:fetch_survey_questions, {:survey_type=>"Cancel"}.merge(args))
end

desc "Clear survey questions, then get them again"
task :refetch_survey_questions, [:survey_type] => [:environment] do |t, args|
  puts "Clearing survey questions and fetching them again...#{args.inspect}"
  SchedulerWorker.perform_async(:refetch_survey_questions, {:survey_type=>"Cancel"}.merge(args))
end
...
desc "Debug log cleanup"
task :debug_log_cleanup => :environment do
  puts "Cleaning Debug logs..."
  SchedulerWorker.perform_async(:debug_log_cleanup)
end

desc "User order cleanup"
task :user_order_cleanup => :environment do
  puts "Cleaning user order history..."
  SchedulerWorker.perform_async(:user_order_cleanup)
end