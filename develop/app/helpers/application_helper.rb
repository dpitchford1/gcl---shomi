module ApplicationHelper
  ...
  def ajax_validation(name, path=nil, form_number=nil)
    path.gsub! /^\//, '' if path
    form_id = form_number ? "#{name}.#{form_number}" : name
    "<script>#{render(:template =>"/pages/validation.js", :layout => nil , :locals => { :name => name, form_id: form_id, path: path || name.to_s.pluralize }).to_s}</script>".html_safe
  end
